...
long Irms0, Irms1, Irms2, Vrms0, Vrms1, Vrms2;
...
void loop() {
  if (timer_count == 1000) {  // 1 second, 
    ...
    for(int d = 0; d < 500; d++){
      ...
      Voltage0_A2[d] = 2*Voltage0_B*cos(angle[d]);
    ...
    for(int e = 0; e < 500; e++){
      ...
      Voltage1_A2[e] = 2*Voltage1_B*cos(angle[e]);
    ...
    for(int f = 0; f < 500; f++){
      ...
      Voltage2_A2[f] = 2*Voltage2_B*cos(angle[f]);
  ...
  // Channel 1
  Channel1_Power = Irms0 * Vrms0 * cos(Current0_theta - Voltage0_theta);

  // Channel 2
  Channel2_Power = Irms1 * Vrms1 * cos(Current1_theta - Voltage1_theta);

  // Channel 3
  Channel3_Power = Irms2 * Vrms2 * cos(Current2_theta - Voltage2_theta);
  }
